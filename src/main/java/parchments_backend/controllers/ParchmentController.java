...
public class ParchmentController {
    ...
    @GetMapping("/core")
    public @ResponseBody ResponseEntity<Object> getCoreParchments(@RequestParam Integer page, @RequestParam String seed) {
        try {
            return ResponseEntity.ok(parchmentService.findCoreParchments(page, seed));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        }
    }
